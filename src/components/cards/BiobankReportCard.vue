      ...
      <div class="row" v-if="biobankDataAvailable && !this.isLoading">
        <div class="col">
          ...
          <div class="container">
            <div class="row">
              <div class="col-md-8">
                <p><strong>Id: </strong>{{ biobank.id }}</p>
                <report-description
  ...
  computed: {
    ...
    collectionsData() {
      return this.biobankDataAvailable && this.biobank.collections
        ? mapCollectionsData(this.biobank.collections).filter(
            it => !it.parentCollection
          )
        : [];
    },
    quality() {
      return {
        Certification: {
          value: mapObjArrayToStringArrayIfExists(this.biobank.quality),
          type: "list"
        }
      };
    }
